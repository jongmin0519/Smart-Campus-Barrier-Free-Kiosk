...
public class MenuFrame extends javax.swing.JFrame implements StateObserver {
  ...
  private void initComponents() {
    ...
    Kiosk_Metro.Main metroPanel = new Kiosk_Metro.Main();
    pnlMetro.add(metroPanel, BorderLayout.CENTER); // metroPanel은 이제 JPanel 인스턴스입니다.
  ...
  private void initFoodPanel() {
	    // 기존에 pnlFood에 있던 모든 컴포넌트를 제거합니다.
	    pnlFood.removeAll();

	    // JavaFX 컨텐츠를 초기화하기 위한 JFXPanel을 생성합니다.
	    final JFXPanel fxPanel = new JFXPanel();
	    
	    // JavaFX 플랫폼에서 실행을 예약합니다.
	    Platform.runLater(() -> {
	        // WebView와 Scene을 생성합니다.
	        WebView webView = new WebView();
	        webView.getEngine().load("http://127.0.0.1:5000");
	        Scene scene = new Scene(webView);
	        
	        // JFXPanel에 Scene을 설정합니다.
	        fxPanel.setScene(scene);
	    });

	    // JFXPanel을 pnlFood 패널에 추가합니다.
	    pnlFood.add(fxPanel);
	    ...
	    // pnlFood의 레이아웃 매니저를 설정하고 크기를 조정할 수 있습니다.
	    // 예를 들어, BorderLayout을 사용하는 경우 다음과 같이 할 수 있습니다.
	    pnlFood.setLayout(new BorderLayout());
	    pnlFood.add(fxPanel, BorderLayout.CENTER);
	}		
